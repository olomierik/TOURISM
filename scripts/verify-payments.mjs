 ...
 * dropped database connection is enough. Eight pay-probe listings from crashed
...
async function sweepStaleFixtures() {
  ...
  try {
    const { rowCount } = await client.query(
      "delete from businesses where slug like 'pay-probe%'",
    );
    const { rows } = await client.query(
      "select id from profiles where email like 'pay-probe-%@example.com'",
    );
